...
for T in range(0,62):
    T = str(T)
    in_dir = '/home/mark/data/img/2016/1jan/T' + T + '/'
    base_name =  'adult_unc63_N2_unc38_ald_1jan_T' + str(T)
    masks = []
    ...
    # Threshold sobel images to bring up worms in binary mask
        ...
        heavy_close = ndimage.morphology.binary_closing(mask1*well_mask, iterations=4)
        x = ndimage.label(heavy_close)
        y= ndimage.find_objects(x[0])
        eggmask = np.zeros_like(img_a)
        for ppd in y:
            darker = False
            row_edge = ppd[0]
            height = 1 + row_edge.stop - row_edge.start
            column_edge = ppd[1]
            width = 1 + ppd[1].stop - ppd[1].start
            obj_mask = x[0] >0
            if np.sum(obj_mask[ppd] * img_a[ppd]) > np.sum(img_a[ppd]):
                darker=True
    
            if height < 18 and width < 18:# and darker is True:
                eggmask[ppd] = 1

        eggmask = eggmask*x[0] >0
...
# save data:
    ...
    np.save(outdir + '/' + outname + 'egg_test.npy', data)
